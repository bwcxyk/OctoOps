...
func TestAlert(c *gin.Context) {
	...
	var err error
	if alert.Type == "email" {
		err = service.SendTestEmail(&alert)
	} else if alert.Type == "dingtalk" {
		err = service.SendTestRobot(&alert)
...
func DeleteAlertGroup(c *gin.Context) {
	id := c.Param("id")
	db.DB.Delete(&model.AlertGroup{}, id)
	c.JSON(http.StatusOK, gin.H{"message": "deleted"})
}
...
func AddAlertGroupMember(c *gin.Context) {
	groupID := c.Param("id")
	var member model.AlertGroupMember
...
func DeleteAlertGroupMember(c *gin.Context) {
	...
	db.DB.Delete(&model.AlertGroupMember{}, memberID)
	c.JSON(http.StatusOK, gin.H{"message": "deleted"})
}
...
func DeleteAlertTemplate(c *gin.Context) {
	id := c.Param("id")
	db.DB.Delete(&model.AlertTemplate{}, id)
	c.JSON(http.StatusOK, gin.H{"message": "deleted"})
}
