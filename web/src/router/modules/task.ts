    ...
    children: [
      {
        ...
        meta: { title: { zh_CN: '调度器状态', en_US: 'Scheduler Status' }, permission: 'task:scheduler:status' },
      },
      {
        path: 'custom',
        name: 'CustomTaskManage',
        component: () => import('@/pages/task/custom/index.vue'),
        meta: { title: { zh_CN: '自定义任务', en_US: 'Custom Task' }, permission: 'task:custom:read' },
      },
      {
        path: 'log',
        name: 'TaskLogManage',
        component: () => import('@/pages/task/log/index.vue'),
        meta: { title: { zh_CN: '任务日志', en_US: 'Task Log' }, permission: 'task:log:read' },
      },
    ],
  },
];
